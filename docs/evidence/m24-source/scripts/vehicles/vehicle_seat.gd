...
func can_mount() -> bool:
	return enabled and not mounted and combat.health.current > 0 and actor.is_on_floor() and actor.dodge_remaining <= 0 and not Input.is_action_pressed("jump") and not Input.is_action_pressed("dodge") and actor.position.distance_to(bike.position) < 3.0 and DamageSystem.clear_line(actor.get_world_3d(), actor.position + Vector3.UP, bike.position + Vector3.UP)
...
func _exit_point() -> Variant:
	...
	for offset in [Vector3(1.7, 0, 0), Vector3(-1.7, 0, 0), Vector3(0, 0, 2.2), Vector3(0, 0, -2.2)]:
		...
		query.exclude = [bike.get_rid(), actor.get_rid()]
