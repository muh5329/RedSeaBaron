...
extends Node3D
const COVER_MASK: int = DamageSystem.COMBAT_COVER_MASK
const SHOT_MASK: int = COVER_MASK | 4
...
func _clear_to(enemy: EnemyActor) -> bool:
	return DamageSystem.clear_line(actor.get_world_3d(), actor.position + Vector3.UP * 1.2, enemy.position + Vector3.UP,COVER_MASK)
...
func fire() -> bool:
	...
	var query := PhysicsRayQueryParameters3D.create(from, from + direction * 120, SHOT_MASK)
	...
	var cover := actor.get_world_3d().direct_space_state.intersect_ray(PhysicsRayQueryParameters3D.create(muzzle, end, COVER_MASK))
