...
func _ready() -> void:
	...
	visual = Node3D.new()
	add_child(visual)
	BlockoutKit.box(visual,Vector3(0,0.65,0),Vector3(1.65,0.2,2),Color("866044"))
	for side in [-1,1]:
		BlockoutKit.box(visual,Vector3(side*0.8,1.06,0),Vector3(0.12,0.65,2),Color("996943"))
		var wheel := BlockoutKit.cylinder(visual,Vector3(side*0.94,0.44,0.25),0.44,0.20,Color("293632"))
		wheel.rotation.z = PI/2
		wheels.append(wheel)
	BlockoutKit.box(visual,Vector3(0,1.06,0.94),Vector3(1.6,0.65,0.12),Color("996943"))
	BlockoutKit.box(visual,Vector3(0,0.53,-1.65),Vector3(0.13,0.13,1.4),Color("8e754d"))
	cargo_visual = Node3D.new()
	visual.add_child(cargo_visual)
