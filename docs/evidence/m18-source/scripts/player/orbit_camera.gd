...
func _unhandled_input(event: InputEvent) -> void:
	...
	if event is InputEventMouseMotion and (Input.mouse_mode == Input.MOUSE_MODE_CAPTURED or drag_look):
		...
		pitch = clampf(pitch - event.relative.y * 0.0025, -1.1, 0.35)

func _physics_process(delta: float) -> void:
	if not is_instance_valid(follow_target):
		return
	if input_enabled:
		yaw -= Input.get_axis("look_left", "look_right") * delta * 1.9
		pitch = clampf(pitch + Input.get_axis("look_down", "look_up") * delta, -1.1, 0.35)
