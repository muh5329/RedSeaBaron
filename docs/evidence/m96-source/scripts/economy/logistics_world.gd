...
var remote_sources: Array[ResourceSource] = []
...
func _ready() -> void:
	for entry in [["Olive orchard","olive",Vector2(-48,-48),true],["Iron outcrop","ore",Vector2(61,29),false],["Timber yard","wood",Vector2(-27,53),false]]:
		...
		# Preserve the original v1 keys, now as explicit stable source identities.
		source.source_id = entry[1]
		source.position = grounded(entry[2])
		source.renewable = entry[3]
		add_child(source)
		sources.append(source)
	var timber := ResourceSource.new()
	timber.source_id = "greenreach_wood"
	timber.item_id = "wood"
	timber.title = "Greenreach timber reserve"
	timber.introduced_in_stock_revision = 2
	timber.position = grounded(Vector2(-3613,716))
	add_child(timber)
	timber.visible = false
	sources.append(timber)
	remote_sources.append(timber)
	var quarry := ResourceSource.new()
	quarry.source_id = "red_mesa_ore"
	quarry.item_id = "ore"
	quarry.title = "Red Mesa iron reserve"
	quarry.initial_stock = 80
	quarry.introduced_in_stock_revision = 3
	quarry.position = grounded(QuarryAdit.SOURCE)
	add_child(quarry)
	quarry.visible = false
	sources.append(quarry)
	remote_sources.append(quarry)
...
func source_by_id(id: String) -> ResourceSource:
...
func refresh_visibility(core_near: bool, point: Vector3) -> void:
	for source in sources: source.visible = point.distance_to(source.position)<110 if source in remote_sources else core_near
	for node: Node3D in stations: node.visible = core_near
...
func use_feedback(target: Node3D, succeeded: bool) -> String:
	if succeeded:
		if target is Storage and target.kind=="delivery":
			return "Delivered · +%d crowns\nReturn to the courier for your next route."%contract.reward
		return "Done · "+hitch.backpack.summary()
	if target is Storage and can_reach(target):
		if target.kind in ["dispatch","delivery"] and contract.state=="COMPLETED":
			return "Delivery already completed\nAt courier: Shift+U repeat · O choose route"
		if target.kind=="dispatch" and contract.state=="IN_TRANSIT":
			return "Finish your current delivery\nDestination: "+selected_route().title
		if target.kind=="delivery" and contract.state=="IN_TRANSIT":
			return "Destination: "+selected_route().title+"\nBring the parcel in your pack or accessible cart."
	return "Approach a resource or desk; check capacity and cargo."
...
func _process(delta: float) -> void:
	...
	if not enabled:
		if nearest!=null: refresh_prompt()
		panel.visible = false
		return
	if toggle:
	...
	if Input.is_action_just_pressed("use_resource"):
		refresh_prompt()
		message.emit(use_feedback(nearest,use_station(nearest,Input.is_action_pressed("sprint"))))
...
func refresh_prompt() -> void:
	nearest = null
	var distance := 4.5
	for target in sources+stations+remote_stations:
		target.label.visible = false
		var d: float = actor.position.distance_to(target.position)
		if d<=distance and target.is_visible_in_tree() and can_reach(target):
			distance = d
			nearest = target
	if nearest!=null:
		nearest.get("label").visible = true
