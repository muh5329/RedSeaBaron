...
func track_asset(kind: String) -> bool:
	...
	waypoint_title = "Motorcycle" if kind=="bike" else "Cargo cart"
...
func refresh_tracking() -> void:
	if not is_instance_valid(tracked_asset): return
...
func _gui_input(event: InputEvent) -> void:
	if event is InputEventMouseButton and event.button_index==MOUSE_BUTTON_LEFT and event.pressed:
		...
		for target: Node3D in [bike,cart]:
			if is_instance_valid(target):
				...
				if distance<threshold:
					...
					threshold = distance
		if closest!=null:
			track_asset("bike" if closest==bike else "cart")
			return
		tracked_asset = null
		waypoint_title = "Waypoint"
