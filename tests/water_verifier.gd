...
func run(owner_game: Node3D) -> void:
	...
	bike.rotation.y = -PI/2
	# The fixture rotates 90 degrees instantly; let the ground visual pose settle.
	await frames(30)
	var flight_mounted := seat.enter()
	await press("transform_vehicle")
	await frames(80)
	check("Coastal flight starts from a grounded, deployed aircraft",flight_mounted and bike.transformation.mode=="AIRCRAFT" and bike.is_on_floor(),"mode=%s rejection=%s"%[bike.transformation.mode,bike.transformation.last_rejection])
	Input.action_press("move_forward")
	await frames(110)
