...
The map shows a red motorcycle marker and a gold cargo-cart marker. Click one, or use Track bike / Track cargo, to follow its current location in the field notes. Remote equipment selects the world map automatically. Clear removes the target; clicking elsewhere sets an ordinary fixed waypoint.
...
Select suites with `tools/verify.sh --suites combat,feedback --no-export`; add `--long` for the roughly twelve-minute continuous flight test. Native flags follow `--`: `--verify` for traversal, or `--verify-SUITE`. Suites include combat, bike, flight, cart, logistics, worker, world, save, ui, integration, feedback, ai, economy, highland, region, recovery, encounter, routine, routine_stop, backdrop, routes, airmail, cargo_access, seat_safety, water, vehicle_ground, chase_camera, batching, equipment_map, enemy_strike, beast_strike, food, lock_framing, shoulder_clearance, panel_combat, wing_pose, touchdown, towing, forest_airmail, source_identity, forest_resource and lake. Web uses `?verify=SUITE`, with `?verify=1` for traversal. A report appears as visible page text after completion. Leave controls idle during automated verification; reload the normal URL afterwards.
