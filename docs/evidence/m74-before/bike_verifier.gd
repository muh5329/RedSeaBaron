...
func run(owner_game: Node3D) -> void:
	...
	await frames(45)
	check("Steering changes heading",absf(bike.rotation.y)>0.5,"yaw=%.2f" % bike.rotation.y)
	release()
	await press("jump",90)
	...
	check("Ground contacts do not erase acceleration at terrain seams",bike.position.z>93 and bike.speed>12,"z=%.2f speed=%.2f"%[bike.position.z,bike.speed])
	release()
	seat.leave(true)
