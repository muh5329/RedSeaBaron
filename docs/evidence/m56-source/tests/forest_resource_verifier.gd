...
func run(owner_game: Node3D) -> void:
	...
	check("Current stock revision records regional depletion",saved.get("stock_revision",1)==2 and saved.stocks.get("source_greenreach_wood",{}).get("wood",0)==57 and SaveSchema.validate(saved,game))
