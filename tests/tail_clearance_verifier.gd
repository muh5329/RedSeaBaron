...
const TAIL_POINTS := {"fin":Vector3(0,2.48,2.4),"stabilizer":Vector3(1.0,1.2,2.4),"propeller":Vector3(0.88,1.45,-1.9),"shaft":Vector3(0,1.08,1.8)}
...
func run(owner_game: Node3D) -> void:
	...
	bike.speed = 0
	await frames(60)
	var cover := BlockoutKit.box(game,bike.position+Vector3(0,1.5,2.7),Vector3(3,3,0.3),Color("87634b"),true)
	await frames(3)
	...
	check("Reset disables every aircraft collider",shapes.size()==8 and shapes.all(func(shape): return shape.disabled) and bike.rider_collision.active and not bike.rider_collision.collision.disabled,"aircraft_shapes=%d rider_active=%s"%[shapes.size(),bike.rider_collision.active])
