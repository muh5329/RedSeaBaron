...
static func populate(chunk: Node3D, coordinate: Vector2i) -> void:
	...
	if material==null:
		...
		shader.code = "shader_type spatial; render_mode cull_disabled; varying vec3 instance_center; void vertex(){instance_center=MODEL_MATRIX[3].xyz;} void fragment(){float visibility=smoothstep(4.0,8.0,distance(instance_center,CAMERA_POSITION_WORLD));float stipple=mod(FRAGCOORD.x+2.0*FRAGCOORD.y,4.0)/4.0;if(stipple>=visibility){discard;}ALBEDO=COLOR.rgb;ROUGHNESS=1.0;}"
		foliage_material = ShaderMaterial.new()
		foliage_material.shader = shader
	for kind: String in counts:
		...
		for i in range(counts[kind]):
			...
			if ground < -1.8 or clearing: continue
