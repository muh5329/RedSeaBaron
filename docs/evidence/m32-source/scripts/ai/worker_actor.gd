...
func _ready() -> void:
	...
	visual.coat_color = Color("4d8b83")
	add_child(visual)
...
func _move_to(delta: float, next: StringName, return_arrival: bool = false) -> bool:
	...
	difference.y = 0
	if difference.length()<4.0 and (abstract_mode or DamageSystem.clear_line(get_world_3d(),position+Vector3.UP,goal+Vector3.UP)):
		desired = Vector3.ZERO
...
func _physics_process(delta: float) -> void:
	...
	if tick_accumulator>=0.1:
		...
		tick_accumulator = 0
		label.text = "%s · %s\n%s"%[worker_name,machine.current,inventory.summary() if inventory.mass()>0 else failure_reason if machine.current in [&"Blocked",&"Failed"] else "Ready" if machine.current==&"Idle" else executor.job.title]
	var from := position
	...
	if machine.current==&"PerformTask": visual.right_arm.rotation.x = -0.8+sin(machine.elapsed*9)*0.5
