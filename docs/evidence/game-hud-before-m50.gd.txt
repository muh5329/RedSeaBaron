...
func _build_map() -> void:
	...
	_label(stack,"Red circle: motorcycle   Gold square: cargo cart",13,MUTED)
	...
	for button in [track_bike_button,track_cart_button,clear_waypoint_button]:
...
func _process(delta: float) -> void:
	if seat:
		...
		movement_controls.text = "WASD  Move     SHIFT  Sprint     SPACE  Jump     Q / CTRL  Dodge" if not seat.mounted else ("W/S  Speed     A/D  Turn     SPACE  Climb     CTRL  Descend" if seat.bike.flight.airborne else "W/S  Drive     A/D  Steer     C  Hitch     V / SHIFT+V  Cargo")
		combat_panel.visible = not seat.mounted and not menu.visible and not map_panel.visible
	if combat and combat.health:
		combat_status.text = "HP %d / 100   |   %s\n%s   %s\n%s" % [combat.health.current, combat.weapon, "PANEL OPEN" if combat.actions_blocked() else combat.state, ("%d / %d ammo" % [combat.ammunition, combat.reserve]) if combat.weapon == "RIFLE" else ("Combo %d" % combat.combo), "R  Reload · Alt  Aim · Tab  Lock" if combat.weapon == "RIFLE" else "F / Click  Swing · G  Heavy · Tab  Lock"]
	...
	if player:
		...
		state_label.text = "WADING" if player.surface_speed_multiplier<1 else player.state
	...
	if hud_clock > 0.25 and player:
		...
		if logistics and game_active:
			...
			var task_text := "%s · %d crowns\n%s\n\n%s\nB  Cargo     N  Worker orders"%[route.title,route.reward,logistics.contract.state.replace("_"," "),"Destination · %.0f m   U  Deliver"%remaining if logistics.contract.state=="IN_TRANSIT" else "Courier desk: O route / U accept"]
			if waypoint!=null:
