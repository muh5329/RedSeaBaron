...
func nearby_container() -> Inventory:
	if hitch.cart.position.distance_to(actor.position)<7: return hitch.cart.inventory
...
func use_station(target: Node3D, reverse: bool = false) -> bool:
	...
	if target is Storage:
		...
		match target.kind:
			...
			"delivery": return contract.deliver(container) or (container!=hitch.backpack and contract.deliver(hitch.backpack))
			"storage":
...
func _process(delta: float) -> void:
	...
	if scan_timer<=0:
		...
		nearest = null
		var distance := 4.5
		for target: Node3D in sources+stations:
	...
	if panel.visible:
		readout.text = "CARGO & TRADE     %d crowns\n\nPack  %.0f / %.0f kg · %s\nCart  %.0f / %.0f kg · %s\nWarehouse · %s\nMarket ore · %d    Rifle reserve · %d / 100\nDelivery · %s (75 crowns) · %d completed\n\nP  Market: 5 rifle rounds / 12 crowns / 1 stocked ore\nShift+U at courier: new parcel after completion\nU  Gather / use desk   ·   Shift+U  Withdraw / buy\nC  Hitch   ·   V  Load pack   ·   Shift+V  Unload\nB  Close     Courier desk near workshop to south quay" % [shop.credits,hitch.backpack.mass(),hitch.backpack.capacity,hitch.backpack.summary(),hitch.cart.inventory.mass(),hitch.cart.inventory.capacity,hitch.cart.inventory.summary(),warehouse.inventory.summary(),shop.stock.count("ore"),combat.reserve,contract.state,contract.deliveries_completed]
