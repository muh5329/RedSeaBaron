...
func run(owner_game: Node3D) -> void:
	...
	if OS.has_feature("web"):
		...
		var quarry: ResourceSource = logi.source_by_id("red_mesa_ore")
		var quarry_stock: Dictionary = quarry.stock.contents()
		quarry.stock.restore_contents({"ore":23})
		...
		quarry.stock.restore_contents(quarry_stock)
