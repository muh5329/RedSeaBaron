...
func _sync_health_target() -> void:
	combat.health.aim_anchor = bike.visual.rider_aim_point if mounted else null
	combat.health.occlusion_receiver = bike.rider_collision.collision if mounted else null
