...
var brook_surface: WaterSurface
...
func _ready() -> void:
	...
	brook_surface = WaterSurface.new()
	brook_surface.water = RegionalBrook.water_body()
	add_child(brook_surface)
	game.player.recovered.connect(func():
		streamer.prepare_focus_collision()
		refresh_tiers()
	)
	# Seat/hitch listeners restore actor and cart positions before this listener runs.
	game.bike.recovered.connect(refresh_tiers)
	game.cart.recovered.connect(refresh_tiers)
