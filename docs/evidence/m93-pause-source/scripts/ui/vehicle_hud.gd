...
func refresh(seat: VehicleSeat) -> void:
	...
	if mode in ["DEPLOYING","FOLDING"]:
		...
		if flying:
			...
			condition.text = "LOW AIRSPEED · Hold W to recover" if slow else (course_label(navigation_target,bike) if navigation_target!=null else "IN FLIGHT · Land before folding or dismounting")
