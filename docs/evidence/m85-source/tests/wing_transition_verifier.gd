...
func run(owner_game: Node3D) -> void:
	...
	cover.queue_free()
	await frames(90)
	var shapes := bike.get_children().filter(func(child): return child is CollisionShape3D)
	check("Clearing cover finishes folding and restores a savable motorcycle",bike.transformation.mode=="BIKE" and shapes.slice(1).all(func(shape): return shape.disabled) and game.saves.can_save())
