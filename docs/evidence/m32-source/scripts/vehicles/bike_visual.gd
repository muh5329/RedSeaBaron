...
func _wheel(parent: Node3D, at: Vector3) -> Node3D:
	...
	VehicleMesh.ring(wheel,Vector3.ZERO,0.30,0.48,0.22,DARK)
	for x in [-0.075,0.075]:
		VehicleMesh.ring(wheel,Vector3(x,0,0),0.295,0.33,0.03,STEEL)
		for i in range(12):
			var angle := i*TAU/12
			VehicleMesh.rod(wheel,Vector3(x*0.5,0,0),Vector3(x,cos(angle)*0.30,sin(angle)*0.30),0.012,STEEL)
	var hub := BlockoutKit.cylinder(wheel,Vector3.ZERO,0.10,0.24,BRASS,-1,12)
	hub.rotation.z = PI/2
	for i in range(24):
		var angle := i*TAU/24
		var tread := BlockoutKit.box(wheel,Vector3(0,cos(angle)*0.469,sin(angle)*0.469),Vector3(0.17,0.02,0.048),DARK)
