...
func fly_toward(goal: Vector2) -> bool:
	...
	while Vector2(bike.position.x,bike.position.z).distance_to(goal)>300 and count<limit:
		...
		for action in ["move_left","move_right","move_back","move_forward"]: Input.action_release(action)
		if absf(heading)>0.025: Input.action_press("move_left" if heading>0 else "move_right")
		if bike.flight.altitude<40: Input.action_press("move_back")
		elif bike.flight.altitude>55: Input.action_press("move_forward")
...
func run(owner_game: Node3D) -> void:
	...
	if "--capture" in OS.get_cmdline_user_args() and DisplayServer.get_name()!="headless":
		# Let the throttled HUD observe the committed delivery before the image.
		await frames(20)
