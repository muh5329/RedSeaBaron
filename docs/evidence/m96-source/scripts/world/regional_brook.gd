...
const LEVEL: float = 6
const WATER_WIDTH: float = 90
const MEANDER := Vector2(12,0.018)
const DECK_HEIGHT: float = 10
static var body: WaterBody

static func channel_x(z: float) -> float:
	return CENTER.x+MEANDER.x*sin((z-CENTER.y)*MEANDER.y)

static func carve(point: Vector2, height: float) -> float:
	var offset := Vector2(absf(point.x-channel_x(point.y)),absf(point.y-CENTER.y))
