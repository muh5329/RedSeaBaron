...
func run(owner_game: Node3D) -> void:
	...
	if "--capture" in OS.get_cmdline_user_args() and DisplayServer.get_name()!="headless":
		...
		get_viewport().get_texture().get_image().save_jpg("res://docs/evidence/highland-native-cruise-m46.jpg")
	release()
	...
	if "--capture" in OS.get_cmdline_user_args() and DisplayServer.get_name()!="headless":
		RenderingServer.force_draw()
		get_viewport().get_texture().get_image().save_jpg("res://docs/evidence/highland-native-stall-hud-m46.jpg")
	Input.action_press("move_forward")
