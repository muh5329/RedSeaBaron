...
func run(owner_game: Node3D) -> void:
	...
	check("Brook rendering and safety share one bounded resource",game.world.brook_surface.water==brook and brook in game.water.additional_waters and brook.bounds.size==Vector2(60,496))
	...
	check("Horizon receives the same channel and end-cap parameters",channel==Vector4(center.x,center.y,8,36) and ends==Vector3(200,48,1))
	...
	await relocate(center+Vector2(0,60),30)
