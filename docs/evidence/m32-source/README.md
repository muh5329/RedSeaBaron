...
Select suites with `tools/verify.sh --suites combat,feedback --no-export`; add `--long` for the roughly twelve-minute continuous flight test. Native flags follow `--`: `--verify` for traversal, or `--verify-SUITE`. Suites include combat, bike, flight, cart, logistics, worker, world, save, ui, integration, feedback, ai, economy, highland, region, recovery, encounter, routine, routine_stop, backdrop, routes, airmail, cargo_access, seat_safety, water, vehicle_ground chase_camera, batching and equipment_map. Web uses `?verify=SUITE`, with `?verify=1` for traversal. A report appears as visible page text after completion. Leave controls idle during automated verification; reload the normal URL afterwards.
