...
func run(owner_game: Node3D) -> void:
	...
	var crew: Array[WorkerActor] = game.workers.workers
	var source_before := logi.source_by_id("wood").stock.count("wood")
	var assigned_all := true
	for member in crew: assigned_all = member.assign(game.workers.make_job("Transport")) and assigned_all
	check("Distant orders accepted for every registered worker",assigned_all)
	var done := await until(func(): return crew.all(func(member): return member.machine.current==&"Idle"),4800)
	check("Distant roster completes independent logistics without physics",done and crew.all(func(member): return member.abstract_mode and member.inventory.mass()==0) and logi.warehouse.inventory.count("wood")==before_stock+5*crew.size() and logi.source_by_id("wood").stock.count("wood")==source_before-5*crew.size(),"workers=%d warehouse=%d"%[crew.size(),logi.warehouse.inventory.count("wood")])
	...
	check("Returning restores worker state and core collision",crew.all(func(member): return not member.abstract_mode and member.inventory.mass()==0 and member.visible) and logi.warehouse.inventory.count("wood")==before_stock+5*crew.size() and game.world.bodies.all(func(b): return b.collision_layer==1))
