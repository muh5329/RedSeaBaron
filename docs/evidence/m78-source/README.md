...
The map shows the motorcycle, cargo cart, Mara and Ivo. Click a marker, or use Track bike / Track cargo / Follow Mara / Follow Ivo, to follow its current location in the field notes. Buttons keep overlapping markers individually selectable. Remote targets select the world map automatically. Clear removes the target; clicking elsewhere sets a fixed waypoint. Tracking is session-only, and follows the same actor when restoring a save in that session.
