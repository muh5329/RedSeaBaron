...
func _physics_process(delta: float) -> void:
	...
	if is_instance_valid(target):
		if target.health.current <= 0 or actor.position.distance_to(target.position) > 30 or not _clear_to(target):
			target = null
	orbit.lateral_offset = 0.7 if weapon == "RIFLE" else 0.0
	if is_instance_valid(target):
		var difference := target.position - actor.position - Vector3.RIGHT.rotated(Vector3.UP, orbit.yaw) * orbit.lateral_offset
