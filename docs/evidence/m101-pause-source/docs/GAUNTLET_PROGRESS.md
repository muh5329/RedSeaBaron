...
## Milestones 94–95 — resumed user requests: aircraft controls, front propeller, terrain and minimap

User explicitly resumes and requests a nose propeller; back/pull-up and forward/descent; Space boost; deploying wings while driving; a minimap and terrain features. This supersedes the pause and prioritizes these requests before the deferred NPC-navigation audit. Preserve m93-pause-source; pre-change vehicle/UI/test/shader sources are in m94-before.

M94: move the propeller to (0,1.45,-1.9), ahead of the wheel, with a separate shaft and collision for both. Keep the shared aircraft assembly transform so deployment, attitude, obstruction and reset stay aligned. Remove only the deployment speed cap, preserving grounded/clearance/cart guards. Use S/back to take off/pull up, W/forward to descend, Space to boost, and Shift as a dedicated aircraft brake. First landing regression was 18/22: front propeller contacted terrain before wheels during steep descent. Add a flare based on a real near-ground ray; retain wheel-support landing requirements. Corrected flight 22/22 and wing-only touchdown 8/8. Tail and camera-cover fixtures needed geometry updates because they referenced the former rear propeller; keep their physical blocked/clear controls. Focused requested controls initially 10/10; brake adds one check, pending final run.

M95: WorldLandforms adds compact physical dune, valley and mountain footprints outside existing settlement locations, mirrored in the distant terrain shader. Snow color now follows summit elevation. TerrainCartography samples actual height/biome/water data with cached relief; FieldMap replaces schematic blocks, and TravelMinimap supplies north-up player-centered metric navigation. Features include the existing quarry cave and beach. Native cartography 13/13, region 18/18, backdrop 7/7, UI 14/14; collision and cache bounds pass. Visual review and Web performance remain required. No rubric increase or acceptance claim yet.

M95 visual review rejected the first layout: expanding field notes overlapped the minimap header, and dark labels disappeared over forest shading. Place the minimap below the actual notes panel, add a non-overlap assertion, and outline map labels. Add village road lines to the minimap. Sharper mountain ridges initially introduced a direction-dependent summit singularity; preserve round2 snowcap/report, taper angular modulation at the apex and regularize radial distance with the same CPU/GPU formula. The original snow-color assertion now passes again, and a new one-meter continuity check prevents summit spikes. Retain the intermediate typed-variable verifier compile failure. Final rendered cartography 15/15, inspected minimap/full-map/dune/valley/snow images. Art remains sparse and procedural.

M94 accepted: focused rendered controls 13/13, including deployment above 15 m/s, continued forward motion, physical front propeller, pull-up/dive, Space boost with zero pitch command, Shift brake, pause, and an upcoming roadside wall (zero penetrating wing samples; safe return to BIKE). Web requested controls 13/13 and full takeoff/landing flight 22/22, clean consoles. Final native regression also completes real Longfield and Greenreach deliveries (1,169.7 m and 3,635.8 m), with correct payment, cargo, landing/folding and ground approaches.

M95 accepted: final native matrix 160/160 across 12 suites; Web matrix 110/110 across seven suites, clean consoles. Web UI 372 calls /120 FPS on this machine. Rendered maps 15/15 and hostile cover 16/16; shader import/export logs clean. M92/M93 pending cover/integration checks are now complete on the updated build. Evidence: m95-acceptance-summary.json, m95-validation and named Web reports. Score stays 7.85.

## Milestone 96 / round 01 — complete immutable regression after user-requested flight and terrain changes

Freeze the accepted source through M95 and run all default suites with source-hash validation. This includes all prior systems plus the new flight_controls and cartography suites. Do not claim acceptance until the complete summary and unchanged hashes are available. M89 remains the latest accepted full checkpoint meanwhile. Continue the separate NPC vehicle-navigation audit against live source only after its own baseline is established.

## Milestone 97 / round 01 — physical NPC vehicle obstruction and moving navigation occupancy

Independent baseline 1/3: the motorcycle physically intersects the navigation cell, but that cell remains open, and an actual hostile body walks through it (31 overlapping frames, ending x=4.526). The raw baseline also has a two-instance shutdown leak warning; preserve it and do not claim clean baseline teardown. Keep the probe and original sources in m97-before. Correct hostile/worker body masks and movement/arrival sight to include vehicles. Navigation samples world+vehicle geometry, tracks bike/cart poses and active shapes at quarter-second intervals, and resamples the deduplicated union of old/new local footprints. Off-grid/airborne obstacles do not trigger unbounded terrain work; removed/disabled bodies retire their old occupancy. Pause gates updates. M96 is frozen before this change and remains untouched. Expanded physical routing, movement/removal recovery and stock-conservation checks are required before acceptance.

M97 initial routing/occupancy challenge passes 12/12: real hostile detour (2.09 m sideways, zero overlap), old/new footprint cleanup, wing deployment/folding, pause/resume, removed-body retirement and actual five-timber worker delivery without duplication. Maximum dynamic sample batch 162. Expanded mounted pursuit then fails 12/13: hostile stalls at the coarse grid's nearest open cell, 5.99 m from the rider, without damage. Preserve m97-mounted-baseline-report.json. For a visible mounted target, choose a nearby standing position using the hostile's actual collision shape, real floor contact, and clear rider sight; cache the bounded candidate search for half a second. Keep ordinary target and memory behavior unchanged. This allows the existing navigation logic to approach a valid attack position instead of the occupied bike center. Pending verification.

M96 failed without source changes: 426/428 checks in the first 26 of 71 suites; all 382 hashes verified. Its water verifier still commanded the obsolete Space/Ctrl pitch inputs, so it never performed the intended coastal flight. Preserve the entire checkpoint and its failure. The same water scenario with revised Back/Forward/Space/Shift input passes 13/13 in an isolated probe (which has a shutdown-only ObjectDB warning); copy the corrected fixture to live source and require a clean registered suite run. Do not mark M96 accepted.

M97 mounted-approach correction passes 13/13: actual pursuit closes from six meters to 2.03 m and lands a 13-damage strike on the exposed rider. Broad worker 25/25, multi-worker 25/25, AI 10/10, raider strike 10/10, beast strike 10/10, combat 34/34 and integrated gameplay 16/16 pass. Add a cold-grid movement probe before acceptance: newly sampled vehicle occupancy must retire correctly if the vehicle moves during initial grid construction.

M97 cold-grid challenge fails 13/14 as intended: moving the bike after its old cell is sampled but before construction completes leaves the old cell blocked. Record initial obstacle positions when each grid is created so its first dynamic reconciliation covers both initial and current footprints, including removal during startup. Preserve m97-cold-grid-baseline-report.json. Add stationary-cache reuse and observational update-time measurement. Registered water suite now passes 13/13 with clean log using the revised controls; no water gameplay code changed.

M97 final focused native 15/15 (31.556 s): initial-grid movement now clears the old footprint, stationary vehicles add zero samples, and the maximum observed batch is 162 cells /0.881 ms. Existing worker, multi-worker, AI, strike, combat and integration regressions passed before the startup-history correction; the fresh full checkpoint will recheck them together. Rendered and Web acceptance remain pending.

## Milestone 98 / round 01 — fresh complete regression after navigation and water-fixture corrections

Freeze current source into a new checkpoint; never edit or relabel failed M96. Run every default native suite, including vehicle_navigation. The user-requested aircraft and map changes remain available in the built game. M97 rendered/Web checks proceed separately against this same source. No full-regression acceptance until all suite results and source hashes verify.

M97 accepted: rendered navigation 15/15, inspected vehicle-navigation-m97.jpg. Web navigation 15/15, water 13/13, integration 16/16 and UI 15/15: 59/59, clean consoles. Largest Web navigation refresh 0.800 ms /162 cells; stationary vehicles add zero samples. Web UI remains 372 calls /120 FPS. M98 remains the separate full checkpoint in progress. Remaining attempt budget 14/20; rubric unchanged at 7.85.

## Milestone 99 / round 01 — exact destination selection from map labels

Adversarial map-input probe fails 0/1: clicking the visible Saffron dunes label creates a generic waypoint 1,923.3 m from the dunes. Preserve the baseline and FieldMap source in m99-before. Use shared label geometry for rendering and hit testing, and map landform/region labels to their exact named coordinates. Preserve equipment tracking priority and arbitrary background waypoint selection. Move the beach destination from x=105 to dry shoreline x=85 before making it directly selectable. Replace the map's implementation-facing “prototype regions” text with an actionable instruction. M98 is frozen before this UI change. Pending input, tracking and Web verification.

M99 accepted: native and rendered focus 17/17; native map/UI/equipment/worker/checkpoint/cartography matrix 84/84. Web landmark 17/17, equipment 12/12, checkpoint 12/12 and UI 15/15: 56/56, clean consoles. Two older background fixtures clicked (7000,3000), only 5.6 map pixels from the selectable dune marker; preserve those failing reports and move the background fixtures to unmarked (7000,5000), retaining exact-coordinate and tracking-release assertions. Inspected ui-native-landmark-destination-m99.jpg: named destination and field-note coordinates agree. Files: FieldMap, WorldLandforms beach destination, HUD scope text, new map_landmark verifier and two fixture coordinates; no scene changes. Debt: static text layout and procedural map detail. Remaining attempt budget 16/20; unchanged rubric.

## Milestone 100 / round 01 — vehicle-driven motion against NPC bodies

Reverse-direction collision baseline fails 0/4: stationary raider/worker bodies do not block wing deployment, the front propeller can grow through a raider, and a driving bike crosses a stationary raider for 21 overlapping frames (ends z=278.067 from z=300, target z=294). Preserve original vehicle sources, fixture and report in m100-before. Use a shared world+hostile+worker mask for vehicle translation, yaw, wing/tail/nose expansion and rider/canopy pose checks. Initial correction passes 4/4. This is physical blocking with no new vehicle-damage mechanic. Expand moving-obstacle, clear-path recovery, death, cart and ownership checks before acceptance. M98 excludes this change. Remaining budget 18/20; unchanged scores.

M100 expanded native 14/14 (20.896 s): stopped bike before raider/worker with zero overlapping frames, removing the blocker restores driving, a worker entering the unfolding span triggers safe folding without penetration, clear space permits full deployment, yaw respects hostile bodies and releases after actual enemy death. Actual mounting/hitching and a 40-ore cart pass through a worker-obstructed route without overlapping or losing cargo; maximum measured drawbar gap 3.513 m. The cart can deflect around the obstacle, which is permitted. No NPC teleport or vehicle damage behavior introduced. Rendered and broad vehicle/worker/combat integration plus Web checks follow. Remaining budget 17/20.

## Milestone 101 / round 01 — intermediate occupancy during navigation startup

M100 final native 303/303 across 17 suites, including multi-worker, full combat-delivery-worker integration and save restoration; rendered focus 14/14 (21.901 s), inspected vehicle-npc-clearance-m100.jpg. One old hostile-cover fixture attempted deployment while its opponent still stood in the front propeller sweep; preserve its 15/16 failure and move the same opponent to its final rear sight-test position before deployment. The original tail-cover/mounted-ownership assertion then passes, alongside combat/integration/save. Web export succeeded; Web collision 14/14 and hostile cover 16/16 pass with clean consoles. Web integration/UI remain pending. Corrected README's stale deployment speed restriction; low-speed folding still applies. Remaining budget 16/20.

M98 accepted: all 1,024/1,024 checks across 72 default suites, return code 0, all 384 frozen source hashes unchanged and independently rechecked. Total suite time 1,760.808 s. This is now the latest complete checkpoint, through M97; M99/M100/M101 are outside its frozen source. Reports and logs are preserved in m98-validation; summary m98-regression-summary.json. This full run does not repeat the optional continuous 23.95 km distance case. Score remains 7.85.

An isolated probe moves the motorcycle to an intermediate location while a new grid samples it, then moves it again before construction finishes. The intermediate cell stays falsely blocked: initial/current footprint cleanup does not cover it. Preserve production source and baseline in m101-before. A temporary candidate records only vehicle-occupied cells observed during initial sampling, then includes them in the first normal reconciliation and clears the temporary history. First candidate failed on an untyped empty array in the underwater branch; preserve the failed candidate/log. Explicit Array[Dictionary] initialization fixes that error and the reproduction passes. Both SceneTree diagnostics still emit two-instance shutdown leak warnings, so neither is clean registered acceptance. Live source remains unchanged during M100 regression/export; integrate only afterward and require normal native/Web checks. Remaining budget 17/20, unchanged rubric.

M101 registered baseline confirms 15/16, with only the intermediate startup location remaining stale. Apply the candidate to NavigationController and retain the new behavioral assertion in vehicle_navigation. Corrected registered suite passes 16/16 (33.4 s), clean log. Full worker/multi-worker/integration regression and Web export follow against unchanged live source. The cleanup reuses actual initial query results and discards temporary cells on first reconciliation; no full-grid second sampling pass is added. Remaining budget 15/20.

M100 accepted: native matrix 303/303, rendered focus 14/14 and Web matrix 61/61 across physical collision, hostile cover, full integration and UI. All final runtime/export logs are clean. Web UI stays at 372 calls /120 FPS on this machine. Evidence: m100-acceptance-summary.json, m100-validation and named Web reports/consoles. No cargo or combat regression; masks preserve existing player ownership. Physics remains kinematic/arcade and art remains blockout, so rubric stays 7.85. Remaining budget 16/20. M101 remains a separate live change pending its full acceptance.

M101 user-requested pause: finish export and prepare the next-session handoff. Final native matrix 82/82 across navigation 16, worker 25, multi-worker 25 and integration 16; clean logs and successful Web export. Final exported Web navigation 16/16 and UI 15/15 pass, clean consoles, 372 calls /120 FPS. The intermediate startup cell is sampled while occupied and becomes clear after relocation. The full M101 Web integrated loop is not replayed during this wrap-up; M100's Web integration 16/16 remains prior-source evidence. Record that exact remaining check in RESUME_HANDOFF.md, archive reports and export/build hashes, and freeze m101-pause-source with a SHA-256 manifest. Restore the normal game URL, leave the local server available, and stop. No M102, automation or agent is started. Remaining M101 budget 15/20; unchanged rubric. The pause snapshot is not a replacement for accepted full checkpoint M98.
