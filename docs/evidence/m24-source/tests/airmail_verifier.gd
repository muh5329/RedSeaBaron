...
func fly_toward(goal: Vector2) -> bool:
	var count := 0
	while Vector2(bike.position.x,bike.position.z).distance_to(goal)>300 and count<5400:
		...
		count += 1
	release()
	return count<5400 and bike.flight.airborne
...
func run(owner_game: Node3D) -> void:
	...
	check("Airmail contract loads selected parcel into workshop cart",logi.route_id=="longfield" and game.cart.inventory.count("package")==1)
	game.orbit.yaw = 0
	...
	check("Airmail physically crosses the streamed world",crossed and bike.travelled-distance_start>900 and not game.world.core_active and game.world.streamer.chunks.size()<=25,"travelled=%.1f position=%s"%[bike.travelled-distance_start,bike.position])
	...
	arrived = await drive_to(Vector2(924,1055),2400)
	arrived = await drive_to(Vector2(924,1116)) and arrived
	arrived = await drive_to(goal) and arrived
	check("Ground approach reaches the visible Longfield desk",arrived and logi.contract.destination.is_visible_in_tree(),"position=%s"%bike.position)
	var crowns := logi.shop.credits
	await press("use_resource")
	check("Integrated flight delivery removes parcel and pays once",logi.contract.state=="COMPLETED" and logi.shop.credits==crowns+180 and game.hitch.backpack.count("package")==0)
	await press("use_resource")
	check("Repeat input cannot repay and distant parked cart stays intact",logi.shop.credits==crowns+180 and game.cart.position.distance_to(cart_at)<0.2 and game.cart.inventory.mass()==0)
	if "--capture" in OS.get_cmdline_user_args() and DisplayServer.get_name()!="headless":
		await RenderingServer.frame_post_draw
		get_viewport().get_texture().get_image().save_jpg("res://docs/evidence/airmail-native-complete.jpg")
	await finish_report("airmail","22 town-to-Longfield flight delivery integration",started,"--verify-airmail")
