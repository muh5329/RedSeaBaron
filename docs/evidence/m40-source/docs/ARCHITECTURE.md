...
| Combat | `HealthComponent`, faction-aware `DamageSystem`, `WeaponConfig`, `PlayerCombat`; timed wrench windows, per-swing hit sets, rifle magazine/reserve/reload, camera + muzzle occlusion checks |
...
| Cargo | `CargoCart` follows the drawbar through world collision. Bike limits outward movement at 4.4 m separation. `HitchSystem` guards speed, range and flight, and routes pack transfers |
...
| Resources | `ResourceSource.stock` is an Inventory, so player and workers use the same stock transaction. `Storage` supplies a spatial inventory endpoint |
...
| Water | Coastal visuals and `WaterSafety` share a `WaterBody` Resource. Three bounded actor queries apply a separate wading multiplier and inventory-preserving recovery; pause freezes countdowns and unsafe water states cannot be saved |
...
- Flight folding is rejected in midair. Deployment needs clear space at both start and completion; obstruction during deployment folds safely. A cart must be detached before deployment.
...
Save version 1 captures grounded/stopped player and vehicles, inventories/stocks, economy, survey flags, hostile health and the one exposed worker’s job stage, regional discoveries, regional encounter records, delivery route and the worker routine phase. Validation rejects malformed records before mutation. Browser persistence has a separate fresh-page reload verifier. Flight saves and a multi-worker saved roster are not supported yet.

## Known limitations

Arcade bike/aircraft/trailer physics; no suspension or realistic aerodynamics. Geometric animation and placeholder models/sounds. Navigation is a coarse local grid, not full-world pathfinding. One authored village and a small catalog. Repeat courier contracts and market-ore ammunition supplies support continued play. No worker wages, touch/controller support or low-end hardware validation. The measured village rendering budget is 335 native draw calls in a pinned village view on one M4 Pro; hidden original meshes remain allocated after batching. Some verifier fixtures directly establish poses; passing them is not exhaustive human playtesting.
