...
func run(owner_game: Node3D) -> void:
	...
	check("Incoming hit sound uses the existing bounded voice pool",game.audio.requests==requests+1 and game.audio.voices.size()==8 and game.audio.get_child_count()==10 and game.audio.wind is WindAmbience and GameAudio.CLIPS.has("hurt"))
