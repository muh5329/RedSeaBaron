...
static func model(kind: String) -> ArrayMesh:
	...
	if kind=="tree":
		RegionalTreeMesh.append_to(surface)
	elif kind=="pine":
...
static func populate(chunk: Node3D, coordinate: Vector2i) -> void:
	...
	for kind: String in counts:
		...
		for i in range(counts[kind]):
			...
			if ground < -1.8 or clearing or RegionalBrook.clearing(at) or RegionalBrook.water_body().depth_at(Vector3(at.x,ground,at.y))>0 or RegionalLake.water_body().depth_at(Vector3(at.x,ground,at.y))>0: continue
	...
	if WorldStreamer.coordinate(Vector3(RegionalBrook.CENTER.x,0,RegionalBrook.CENTER.y))==coordinate: RegionalBrook.build_bridge(chunk)
...
static func landmark(chunk: Node3D, region: RegionDefinition) -> void:
	...
	if region.biome=="Highlands":
		...
		BlockoutKit.box(root,Vector3(0,10.3,0),Vector3(5.5,0.6,5.5),Color("d1d5ca"),true)
		...
		BlockoutKit.mesh(root,roof,Vector3(0,4.6,0),region.accent,true)
		for i in range(5):
