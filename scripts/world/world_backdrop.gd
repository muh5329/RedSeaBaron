...
func _ready() -> void:
	...
	geometry.mesh = plane
	geometry.extra_cull_margin = 400
	geometry.cast_shadow = GeometryInstance3D.SHADOW_CASTING_SETTING_OFF
	material = ShaderMaterial.new()
	material.shader = preload("res://assets/shaders/world_backdrop.gdshader")
	material.set_shader_parameter("lake_basin",Vector4(RegionalLake.CENTER.x,RegionalLake.CENTER.y,RegionalLake.INNER_RADIUS,RegionalLake.OUTER_RADIUS))
	material.set_shader_parameter("lake_bottom",RegionalLake.BOTTOM)
	material.set_shader_parameter("brook_channel",Vector4(RegionalBrook.CENTER.x,RegionalBrook.CENTER.y,RegionalBrook.INNER_WIDTH,RegionalBrook.OUTER_WIDTH))
	material.set_shader_parameter("brook_meander",RegionalBrook.MEANDER)
	material.set_shader_parameter("brook_ends",Vector3(RegionalBrook.HALF_LENGTH,RegionalBrook.END_BLEND,RegionalBrook.BOTTOM))
