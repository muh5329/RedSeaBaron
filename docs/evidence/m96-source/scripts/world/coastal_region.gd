...
static var roof_material: ShaderMaterial
var merged_visuals: int = 0
var camera_obstacle: MeshInstance3D
@export var sea := WaterBody.new()
...
static func height_at(x: float, z: float) -> float:
	...
	return RegionalBrook.carve(Vector2(x,z),RegionalLake.carve(Vector2(x,z),lerpf(local,far,blend)+WorldLandforms.offset(Vector2(x,z))))

static func core_ground_height(x: float, z: float) -> float:
	# Match the authored terrain's triangle interpolation, not just its height function.
	var gx := floorf(x/GRID)*GRID
	var gz := floorf(z/GRID)*GRID
	var u := (x-gx)/GRID
	var v := (z-gz)/GRID
	var a := height_at(gx,gz)
	var b := height_at(gx+GRID,gz)
	var c := height_at(gx,gz+GRID)
	if u+v<=1: return a+(b-a)*u+(c-a)*v
	var d := height_at(gx+GRID,gz+GRID)
...
func _roads() -> void:
	...
	_plaza(Vector2(2,2),12)

func _plaza(center: Vector2, radius: float) -> void:
	var surface := SurfaceTool.new()
	surface.begin(Mesh.PRIMITIVE_TRIANGLES)
	var rings := int(ceilf(radius))
	for r in range(rings):
		for i in range(48):
			var angle := i*TAU/48
			var next := (i+1)*TAU/48
			var a := center+Vector2(cos(angle),sin(angle))*radius*r/rings
			var b := center+Vector2(cos(angle),sin(angle))*radius*(r+1)/rings
			var c := center+Vector2(cos(next),sin(next))*radius*r/rings
			var d := center+Vector2(cos(next),sin(next))*radius*(r+1)/rings
			var points := PackedVector2Array([a,b,d] if r==0 else [a,b,c,b,d,c])
			for point in points: surface.add_vertex(Vector3(point.x,core_ground_height(point.x,point.y)+0.04,point.y))
	surface.generate_normals()
	var plaza := BlockoutKit.mesh(self,surface.commit(),Vector3.ZERO,Color("c7b28b"))
...
func _building(at: Vector2, size: Vector3, color: Color, title: String) -> void:
	...
	var roof_visual := BlockoutKit.mesh(root, roof, Vector3(0, size.y + 1.3, 0), Color("9d5542"), true)
	if roof_material==null:
		roof_material = ShaderMaterial.new()
		roof_material.shader = preload("res://assets/shaders/terracotta_roof.gdshader")
	roof_visual.material_override = roof_material
	# This shader deliberately uses world coordinates, so static merging is safe.
	roof_visual.set_meta("batchable_world_shader",true)
	for side in [-1,1]:
		BlockoutKit.box(root,Vector3(side*(size.x+1.2)*0.5,size.y-0.12,0),Vector3(0.14,0.18,size.z+1.2),Color("6c4e39"))
		BlockoutKit.box(root,Vector3(0,size.y-0.12,side*(size.z+1.2)*0.5),Vector3(size.x+1.2,0.18,0.14),Color("6c4e39"))
	...
	for side in [-1,1]:
		BlockoutKit.box(root,Vector3(side*0.76,1.25,front+0.10),Vector3(0.16,2.5,0.20),Color("e0c6a3"))
	BlockoutKit.box(root,Vector3(0,2.55,front+0.10),Vector3(1.68,0.18,0.20),Color("e0c6a3"))
	for x in [-0.39,-0.13,0.13,0.39]:
		BlockoutKit.box(root,Vector3(x,1.2,front+0.086),Vector3(0.018,2.25,0.015),Color("304e50"))
	BlockoutKit.sphere(root,Vector3(0.43,1.12,front+0.16),Vector3.ONE*0.12,Color("d3b381"))
	for x in [-size.x * 0.33, size.x * 0.33]:
		...
		BlockoutKit.box(root,Vector3(x,2.25,front+0.2),Vector3(1.1,0.07,0.1),Color("d3b381"))
		BlockoutKit.box(root,Vector3(x,1.40,front+0.14),Vector3(1.7,0.16,0.42),Color("e0c6a3"))
		for side in [-1,1]:
			BlockoutKit.box(root,Vector3(x+side*0.96,2.25,front+0.09),Vector3(0.38,1.55,0.12),Color("435f59"))
			for y in [1.8,2.65]:
				BlockoutKit.box(root,Vector3(x+side*0.96,y,front+0.16),Vector3(0.38,0.07,0.035),Color("6c4e39"))
...
func _rock(at: Vector3, size: Vector3, color: Color, collision: bool = false) -> MeshInstance3D:
	var surface := SurfaceTool.new()
	surface.begin(Mesh.PRIMITIVE_TRIANGLES)
	...
	for level in range(3):
		for j in range(7):
			var next := (j + 1) % 7
			var band := 0.76+0.15*(0.5+0.5*sin(j*2.7+level*1.9))
			surface.set_color(color*Color(band,band,band,1))
			for v in [rings[level][j], rings[level][next], rings[level + 1][j], rings[level][next], rings[level + 1][next], rings[level + 1][j]]:
				surface.add_vertex(v)
	for j in range(7):
		surface.set_color(color.darkened(0.08))
		for v in [Vector3(0, 0.5, 0), rings[3][j], rings[3][(j + 1) % 7]]:
			surface.add_vertex(v)
	for j in range(7):
		surface.set_color(color.darkened(0.22))
		for v in [Vector3(0,-0.5,0),rings[0][(j+1)%7],rings[0][j]]:
			surface.add_vertex(v)
	surface.generate_normals()
	var item := BlockoutKit.mesh(self, surface.commit(), at, color)
	if rock_material==null:
		rock_material = StandardMaterial3D.new()
		rock_material.roughness = 1
		rock_material.vertex_color_use_as_albedo = true
		rock_material.vertex_color_is_srgb = true
	item.material_override = rock_material
	item.scale = size
...
func _landmarks() -> void:
	...
	for i in range(21):
		...
		# Keep the original rock RNG consumption so later collision landmarks stay fixed.
		var crown := _rock(Vector3(x, height_at(x, z) + h * 0.77 + 0.5, z), Vector3(13, 4.5, 11), Color("5d7762"))
		crown.scale.y = 0.24
		crown.position.y = height_at(x,z)+h*0.77+0.08
		CliffGrove.build(self,Vector3(x,height_at(x,z)+h*0.77+0.12,z),1947+i*313)
	for i in range(16):
		var z := -120.0 + i * 15.0
...
func _multimesh(geometry: Mesh, transforms: Array[Transform3D], color: Color, camera_fade: bool = false) -> void:
	...
	node.material_override = FoliageFade.material(color) if camera_fade else BlockoutKit.material(color)
	add_child(node)
...
func _vegetation() -> void:
	...
	_multimesh(trunk, trunks, Color("756047"),true)
	var foliage := SphereMesh.new()
	foliage.radial_segments = 7
	foliage.rings = 3
	_multimesh(foliage, crowns, Color("4d7160"),true)
	...
	for point in [Vector2(-9, 19), Vector2(15, 13), Vector2(-28, -26), Vector2(29, -45), Vector2(-48, -73)]:
		...
		var stem := BlockoutKit.cylinder(self, base + Vector3.UP * 1.3, 0.21, 2.6, Color("706449"))
		stem.material_override = FoliageFade.material(Color("706449"))
		var cypress := BlockoutKit.sphere(self, base + Vector3.UP * 5.2, Vector3(2.1, 5.3, 2.1), Color("566c43"))
		cypress.material_override = FoliageFade.material(Color("566c43"))
