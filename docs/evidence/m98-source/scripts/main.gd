...
var regional_sites: Array[SurveySite] = []
...
func _ready() -> void:
	...
	for destination: RegionDefinition in RegionCatalog.all():
		var board := RegionalSurveySite.new()
		board.actor = player
		board.combat = combat
		board.site_id = destination.id
		board.title = destination.title
		board.description = "Checkpoint set: "+destination.title+". On-foot recovery returns here."
		var at := destination.center+Vector2(-12,8)
		board.position = Vector3(at.x,CoastalRegion.height_at(at.x,at.y)+0.08,at.y)
		add_child(board)
		regional_sites.append(board)
	interaction = InteractionController.new()
	interaction.actor = player
	interaction.sites.assign(sites+regional_sites)
	add_child(interaction)
	hud = GameHUD.new()
	hud.audio_preferences = audio_preferences
	hud.seat = seat
	...
	cart.message.connect(hud.notify)
	...
	interaction.interacted.connect(func(site: SurveySite):
		...
		player.spawn_position = site.recovery_position()
		hud.show_prompt(site)
	...
	water.additional_waters.append(RegionalBrook.water_body())
	water.player = player
	water.bike = bike
	water.cart = cart
	water.seat = seat
...
func _set_paused(value: bool) -> void:
	...
	for nav in get_tree().get_nodes_in_group("navigation_grids"): nav.dynamic_enabled = not value
	...
	hud.incoming_feedback.set_active(not value)
