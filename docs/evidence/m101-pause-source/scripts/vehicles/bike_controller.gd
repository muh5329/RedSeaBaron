...
func _ready() -> void:
	...
	collision_layer = 8
	collision_mask = VehicleClearance.WORLD_AND_NPCS
...
func try_turn(yaw: float) -> bool:
	...
	return VehicleClearance.apply_turn(self,yaw,collision_mask | (16 if cart_attached else 0),not flight.airborne)
