...
func run(owner_game: Node3D) -> void:
	...
	if "--capture" in OS.get_cmdline_user_args() and DisplayServer.get_name()!="headless":
		...
		game.add_child(view)
		view.global_position = bike.position+Vector3(4,2.8,3)
		view.look_at(bike.position+Vector3.UP*1.3)
		view.current = true
		await frames(2)
		RenderingServer.force_draw()
		get_viewport().get_texture().get_image().save_jpg("res://docs/evidence/rider-clearance-m86.jpg")
		view.queue_free()
	...
	var post := BlockoutKit.box(game,bike.position+Vector3(-0.50,1.68,-0.03),Vector3(0.025,0.04,0.025),Color("655645"),true)
	await frames(3)
	var before_clear := VehicleClearance.turn_clear(bike,0,1)
	var proposal_clear := VehicleClearance.turn_clear(bike,0.25,1)
	var turned := bike.try_turn(0.25)
	check("Upper rider corner prevents a turn into a post",before_clear and not proposal_clear and not turned and absf(bike.rotation.y)<0.001,"before_clear=%s proposal_clear=%s yaw=%.4f"%[before_clear,proposal_clear,bike.rotation.y])
	post.queue_free()
	await frames(3)
	check("Removing rider-height obstruction releases steering",bike.try_turn(0.25) and absf(bike.rotation.y-0.25)<0.001)
	bike.rotation = Vector3.ZERO
	post = BlockoutKit.box(game,bike.position+Vector3(-0.45,2.10,-0.16),Vector3(0.035,0.06,0.06),Color("655645"),true)
	await frames(3)
	bike.visual.rotation.z = 0.15
	proposal_clear = bike.rider_collision.can_fit(bike.visual.transform)
	bike.rider_collision.restrict_pose(Transform3D.IDENTITY)
	bike.rider_collision.sync_transform()
	check("Visual bank cannot push the rider through an obstacle",not proposal_clear and bike.visual.transform.is_equal_approx(Transform3D.IDENTITY) and bike.rider_collision.collision.transform.is_equal_approx(bike.visual.transform),"proposal_clear=%s roll=%.4f"%[proposal_clear,bike.visual.rotation.z])
	post.queue_free()
	await frames(3)
	bike.visual.rotation.z = 0.15
	bike.rider_collision.restrict_pose(Transform3D.IDENTITY)
