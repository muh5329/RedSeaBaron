...
static func fender(parent: Node3D, center: Vector3, color: Color, start: float, end: float, radius: float = 0.54) -> void:
	# Short overlapping curved strips make a readable faceted steel mudguard.
	for i in range(10):
		var angle := lerpf(start,end,(i+0.5)/10.0)
		var strip := BlockoutKit.box(parent,center+Vector3(0,cos(angle)*radius,sin(angle)*radius),Vector3(0.31,0.045,radius*(end-start)/10.0+0.025),color)
		strip.rotation.x = angle

static func wheel(parent: Node3D, at: Vector3, radius: float, width: float, tire_color: Color, steel: Color, brass: Color) -> Node3D:
	var wheel := Node3D.new()
	wheel.position = at
	parent.add_child(wheel)
	ring(wheel,Vector3.ZERO,radius*0.625,radius,width,tire_color)
	for x in [-width*0.34,width*0.34]:
		ring(wheel,Vector3(x,0,0),radius*0.615,radius*0.6875,0.03,steel)
		for i in range(12):
			var angle := i*TAU/12
			rod(wheel,Vector3(x*0.5,0,0),Vector3(x,cos(angle)*radius*0.625,sin(angle)*radius*0.625),0.012,steel)
	var hub := BlockoutKit.cylinder(wheel,Vector3.ZERO,0.10,width+0.02,brass,-1,12)
	hub.rotation.z = PI/2
	for i in range(24):
		var angle := i*TAU/24
		var tread := BlockoutKit.box(wheel,Vector3(0,cos(angle)*(radius-0.011),sin(angle)*(radius-0.011)),Vector3(width*0.77,0.02,0.048),tire_color)
		tread.rotation.x = angle
	StaticBatcher.combine(wheel)
	return wheel
