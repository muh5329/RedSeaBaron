...
| Input | Action |
|---|---|
| WASD / arrows | Move; ground drive; airborne S/back pulls up, W/forward dives |
| Shift | Sprint; aircraft brake; reverse cargo/station action when held |
| Space | Jump; bike brake; aircraft boost |
| Q / Ctrl | Dodge on foot |
...
| T | Deploy wings while driving on clear ground; land and slow below 29 km/h to fold |
...
The map shows the motorcycle, cargo cart, Mara and Ivo. Click a marker, or use Track bike / Track cargo / Follow Mara / Follow Ivo, to follow its current location in the field notes. Buttons keep overlapping markers individually selectable. Remote targets select the world map automatically. The Checkpoint button follows your selected recovery point, including changes made at survey boards or by loading a save. A ring marks it on the map. Click a landform or region label to select its exact destination. Clear removes the target; clicking empty map space sets a fixed waypoint. Tracking is session-only, and follows the same actor when restoring a save in that session.
...
The motorcycle and cart start at the workshop, east of the village center. Deploy wings, accelerate above 44 km/h, then hold S / back to take off. Land and slow down before folding. Detach the ground cart before flight. Wings may deploy while driving on clear ground. In flight, S/back pulls up, W/forward dives, Space boosts, and Shift brakes. The propeller sits ahead of the front wheel; its sweep and shaft have collision.
...
The north-up minimap follows the traveler and shows nearby equipment, checkpoints and the selected waypoint. The full map uses terrain relief and biome colors. Explore Saffron dunes (6600, 3000), Windcut valley (-800, -6500), Crown mountains (-1800, -8600) and Whitecap peak (1100, -10300). Snow covers the higher summits. Solis beach and the existing Red Mesa quarry cave are marked as well.

Hostiles and workers now physically avoid the motorcycle. Navigation updates when vehicles move or wings change shape, including during startup, and clears retired footprints. Hostiles can still approach and strike an exposed mounted rider.
# RedSeaBaron
