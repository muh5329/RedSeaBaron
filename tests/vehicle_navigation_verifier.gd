...
func run(owner_game: Node3D) -> void:
	...
	cold.queue_free()
	await frames(20)
	# A second move during construction must also retire the intermediate cell.
	bike.position = ground(Vector2(-100,80))
	var history := NavigationController.new()
	game.add_child(history)
	var middle := ground(Vector2(0,80))
	var middle_cell := history.coordinate(middle)
	var middle_index := middle_cell.y*history.side_cells+middle_cell.x
	while history.cursor<middle_index-192: await frames(1)
	bike.position = middle
	while history.cursor<=middle_index: await frames(1)
	var sampled_middle := history.grid.is_point_solid(middle_cell)
	bike.position = ground(Vector2(100,80))
	await frames(45)
	check("Multiple startup moves retire intermediate vehicle occupancy",sampled_middle and history.ready_for_paths and not history.grid.is_point_solid(middle_cell) and history.grid.is_point_solid(history.coordinate(bike.position)),"intermediate sampled=%s stale=%s"%[sampled_middle,history.grid.is_point_solid(middle_cell)])
	history.queue_free()
	bike.position = ground(Vector2(0,80))
	await frames(20)
	...
	await finish_report("vehicle_navigation","101 startup and physical NPC vehicle navigation",started,"--verify-vehicle_navigation")
