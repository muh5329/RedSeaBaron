...
var sea: WaterBody
var additional_waters: Array[WaterBody] = []
...
func safe_to_save() -> bool:
	return depth_at(player.position)<=1.25 and depth_at(bike.position)<=0.55 and depth_at(cart.position)<=1.0

func depth_at(at: Vector3) -> float:
	var depth := sea.depth_at(at)
	for body in additional_waters: depth = maxf(depth,body.depth_at(at))
	return depth

func _physics_process(delta: float) -> void:
	if not enabled: return
	player_depth = 0 if seat.mounted else depth_at(player.position)
	...
	bike_timer = bike_timer+delta if depth_at(bike.position)>0.55 else 0.0
	if bike_timer>=0.35:
		bike_recoveries += 1
		bike.recover()
		message.emit("Waterlogged motorcycle · returned to the workshop with attached cargo.")
	cart_timer = cart_timer+delta if not cart.tow_vehicle and depth_at(cart.position)>1.0 else 0.0
	if cart_timer>=2:
		cart_recoveries += 1
