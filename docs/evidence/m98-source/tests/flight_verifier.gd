...
func capture_flight(label: String) -> void:
	if "--capture" in OS.get_cmdline_user_args() and DisplayServer.get_name()!="headless":
		...
		for argument in OS.get_cmdline_user_args():
			if argument.begins_with("--capture-prefix="): tag=argument.trim_prefix("--capture-prefix=")
		RenderingServer.force_draw()
		get_viewport().get_texture().get_image().save_jpg("res://docs/evidence/flight-native-"+label+"-"+tag+".jpg")
...
func run(owner_game: Node3D) -> void:
	...
	await capture_flight("runway-hud")
	await press("move_back",1)
	...
	await capture_flight("ready-hud")
	...
	await capture_flight("climb-hud")
	release()
	...
	await capture_flight("descent-hud")
	var wait_frames := 0
