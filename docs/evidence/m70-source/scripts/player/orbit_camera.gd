...
func _physics_process(delta: float) -> void:
	...
	var base := follow_target.global_position+Vector3.UP*1.65
	var right := Vector3.RIGHT.rotated(Vector3.UP,yaw)
	var desired := _clear_pivot(base,base+right*lateral_offset)
	# Clamp again after smoothing so newly adjacent cover is resolved this frame.
	global_position = _clear_pivot(base,global_position.lerp(desired,1-exp(-18*delta)))
	effective_lateral_offset = (global_position-base).dot(right)
	rotation = Vector3(pitch, yaw, 0.0)
	arm.spring_length = lerpf(arm.spring_length, distance, 1.0 - exp(-9.0 * delta))

func _clear_pivot(base: Vector3, desired: Vector3) -> Vector3:
	if base.distance_squared_to(desired)<0.000001: return desired
	...
	return base.lerp(desired,fractions[0])

func snap() -> void:
	if is_instance_valid(follow_target):
		var base := follow_target.global_position+Vector3.UP*1.65
		var right := Vector3.RIGHT.rotated(Vector3.UP,yaw)
		global_position = _clear_pivot(base,base+right*lateral_offset)
		effective_lateral_offset = (global_position-base).dot(right)
	rotation = Vector3(pitch, yaw, 0.0)
