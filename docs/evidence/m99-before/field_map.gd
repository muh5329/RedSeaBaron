...
func _draw() -> void:
	...
	if world_view:
		...
		for region in RegionCatalog.all():
			...
			_caption(p+offsets[region.id],region.title.split(" ")[0],12)
	...
	if world_view:
		for feature in WorldLandforms.landmarks():
			if feature[2]=="Beach" or feature[2]=="Cave": continue
			var at := point(Vector3(feature[1].x,0,feature[1].y))
			_caption(at+Vector2(8,14),feature[0],10)
		var cave := point(QuarryAdit.position_3d())
		draw_line(cave,cave+Vector2(25,-25),Color("394f47"),1)
		_caption(cave+Vector2(26,-26),"△ Quarry cave",11)
		_caption(Vector2(91,331),"FOREST              GRASSLAND              DESERT",10)
	else:
		mark(Vector3(105,0,60),"Beach",Color("ead1a4"),3)
