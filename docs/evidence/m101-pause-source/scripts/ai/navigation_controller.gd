...
var startup_obstacle_cells: Dictionary = {}
...
func _sample(cell: Vector2i) -> void:
	...
	if y>= -1.8: hits = get_world_3d().direct_space_state.intersect_shape(query,1)
	# A vehicle can visit multiple locations before the initial grid is ready.
	# Retain only cells where this sampling pass actually observed its body.
	if not ready_for_paths and not hits.is_empty() and hits[0].collider.is_in_group("navigation_obstacles"):
		startup_obstacle_cells[cell] = true
	var blocked := y < -1.8 or not hits.is_empty()
...
func refresh_obstacles() -> void:
	...
	var dirty: Dictionary = startup_obstacle_cells.duplicate()
	startup_obstacle_cells.clear()
	for body: Node3D in get_tree().get_nodes_in_group("navigation_obstacles"):
